...
router.post('/convert-to-image', async (req, res) => {
    try {
        // HTML-indhold, der skal konverteres til billede
        const htmlContent = `
        <!DOCTYPE html>
<html lang="en">
<head>
...
<header>
  ...
  function convertHtmlToImage() {
      ...
      .catch(error => {
        ...
        // Opretter en ny browserinstans
        const browser = await puppeteer.launch();

        // Opretter en ny side
        const page = await browser.newPage();

        // Indsætter HTML-indholdet i siden
        await page.setContent(htmlContent);

        // Generer et unikt filnavn for billedet
        const imageName = `image-${uuidv4()}.png`;

        // Sti til, hvor billedet skal gemmes
        const imagePath = path.join(__dirname, '..', 'public', 'images', imageName);

        // Tager et screenshot af siden og gemmer det som et billede
        await page.screenshot({ path: imagePath });

        // Lukker browseren
        await browser.close();

        // Sender stien til det gemte billede til klienten
        res.status(200).json({ imageUrl: `/images/${imageName}` });
    } catch (error) {
        console.error('Fejl under konvertering af HTML til billede:', error);
        res.status(500).json({ error: 'Der opstod en fejl under konvertering af HTML til billede' });
    }
});
